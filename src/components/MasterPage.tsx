...
import { Container, Box, Typography, TextField, AppBar, Toolbar, Accordion, AccordionSummary, AccordionDetails, Alert, Button } from '@mui/material';
  ...
  const handleFunctionCall = async (func: any) => {
    ...
    try {
      const args = func.inputs.map((input: any, index: number) => {
        const value = (document.getElementById(`${func.name}-${index}`) as HTMLInputElement).value;
        return func.inputs[index].type === 'uint256' ? BigInt(value) : value;
      });
      const result = await contract[func.name](...args);
      setResults(prevResults => ({
        ...prevResults,
        [func.name]: JSON.stringify(result, (key, value) => (typeof value === 'bigint' ? value.toString() : value))
      }));
    } catch (error: unknown) {
            ...
            <AccordionDetails className="accordion-details">
              <Box sx={{ marginBottom: 2 }}>
                {item.inputs.map((input: any, index: number) => (
                  <TextField key={index} id={`${item.name}-${index}`} label={input.name} variant="outlined" sx={{ marginBottom: 1 }} fullWidth />
                ))}
                <Button variant="contained" onClick={() => handleFunctionCall(item)} sx={{ marginTop: 2 }}>
                  Call {item.name}
                </Button>
                {results[item.name] && (
